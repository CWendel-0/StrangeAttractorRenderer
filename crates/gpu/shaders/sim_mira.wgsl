// GPU-side Mira attractor (discrete map, 2-D).
//
// Params in p[]:
//   p[0].x = a, p[0].y = b
//
// f(t) = a*t + 2*(1-a)*t^2 / (1+t^2)
// x' = b*y + f(x)
// y' = -x + f(x')
// z' = 0
...
fn main(@builtin(global_invocation_id) gid: vec3<u32>) {
    ...
    for (var i = 0u; i < params.steps; i++) {
        let nx = b * y + mira_f(x, a);
        let ny = -x + mira_f(nx, a);
        ...
        if abs(x) > 1e6 || abs(y) > 1e6 {
            x = 12.0 + (recovery_hash(traj) - 0.5) * 0.5;
            y = (recovery_hash(traj ^ 0x9E3779B9u) - 0.5) * 0.5;
            for (var w = 0u; w < 200u; w++) {
                let wx = x; let wy = y;
                let wnx = b * wy + mira_f(wx, a);
                x = wnx;
                y = -wx + mira_f(wnx, a);
            }
            continue;
