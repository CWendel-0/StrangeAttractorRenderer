// GPU-side Modified Lozi attractor (discrete map, 2-D).
//
// Params in p[]:
//   p[0].x = a, p[0].y = b
//
...
fn main(@builtin(global_invocation_id) gid: vec3<u32>) {
    ...
    for (var i = 0u; i < params.steps; i++) {
        ...
        if abs(x) > 1e6 || abs(y) > 1e6 {
            x = (recovery_hash(traj) - 0.5) * 1.0 + 0.5;
            y = (recovery_hash(traj ^ 0x9E3779B9u) - 0.5) * 1.0 + 0.5;
            for (var w = 0u; w < 200u; w++) {
                let wx = x; let wy = y;
                x = 1.0 + a * (abs(wx) - wy * wy) + wy;
                y = b * wx;
            }
            continue;
        }
