// GPU-side Qi-3D attractor simulation.
//
// Params in p[]:
...
fn main(@builtin(global_invocation_id) gid: vec3<u32>) {
    ...
    for (var i = 0u; i < params.steps; i++) {
        ...
        if abs(x) > 1e6 || abs(y) > 1e6 || abs(z) > 1e6 {
            x = 3.0 + (recovery_hash(traj) - 0.5) * 0.2;
            y = -4.0 + (recovery_hash(traj ^ 0x9E3779B9u) - 0.5) * 0.2;
            z = 3.0 + (recovery_hash(traj ^ 0x85EBCA6Bu) - 0.5) * 0.2;
            for (var w = 0u; w < 500u; w++) {
                let wx = x; let wy = y; let wz = z;
                let dvx = a * (wy - wx) + e * wy * wz;
                let dvy = c * wx + d * wy - wx * wz;
                let dvz = wx * wy - b * wz;
                x = wx + dt * dvx;
                y = wy + dt * dvy;
                z = wz + dt * dvz;
