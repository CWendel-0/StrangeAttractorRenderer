// GPU-side Popcorn attractor (discrete map, 2-D).
//
// Params in p[]:
//   p[0].x = a, p[0].y = b
//
...
fn main(@builtin(global_invocation_id) gid: vec3<u32>) {
    ...
    for (var i = 0u; i < params.steps; i++) {
        let nx = a * sin(y + tan(b * y));
        let ny = a * sin(x + tan(b * x));
        ...
        if abs(x) > 1e6 || abs(y) > 1e6 {
            x = (recovery_hash(traj) - 0.5) * 0.5 + 0.6;
            y = (recovery_hash(traj ^ 0x9E3779B9u) - 0.5) * 0.5 + 0.2;
