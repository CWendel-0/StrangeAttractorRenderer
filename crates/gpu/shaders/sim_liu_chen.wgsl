// GPU-side Liu-Chen attractor simulation.
//
// Params in p[]:
//   p[0] = (a, b, c, d)
//   p[1] = (e, f, g, dt)
...
fn main(@builtin(global_invocation_id) gid: vec3<u32>) {
    ...
    for (var i = 0u; i < params.steps; i++) {
        ...
        if abs(x) > 1e6 || abs(y) > 1e6 || abs(z) > 1e6 {
            ...
            y = 3.0 + (recovery_hash(traj ^ 0x9E3779B9u) - 0.5) * 0.2;
            z = 5.0 + (recovery_hash(traj ^ 0x85EBCA6Bu) - 0.5) * 0.2;
            for (var w = 0u; w < 500u; w++) {
                let wx = x; let wy = y; let wz = z;
                x = wx + dt * (a*wy + b*wx + c*wy*wz);
                y = wy + dt * (d*wy - wz + e*wx*wz);
                z = wz + dt * (f*wz + g*wx*wy);
            }
            continue;
        }
