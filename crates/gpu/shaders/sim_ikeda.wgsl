// GPU-side Ikeda attractor simulation.
//
// Params in p[]:
//   p[0].x = c, p[0].y = u
//
...
fn main(@builtin(global_invocation_id) gid: vec3<u32>) {
    ...
    for (var i = 0u; i < params.steps; i++) {
        ...
        let nx = 1.0 + u * (x * cos(t) - y * sin(t));
        let ny = u * (x * sin(t) + y * cos(t));
        ...
        if abs(x) > 1e6 || abs(y) > 1e6 {
            ...
            for (var w = 0u; w < 200u; w++) {
                let wx = x; let wy = y;
                let wt = c - 6.0 / (1.0 + wx * wx + wy * wy);
                x = 1.0 + u * (wx * cos(wt) - wy * sin(wt));
                y = u * (wx * sin(wt) + wy * cos(wt));
            }
            continue;
        }
