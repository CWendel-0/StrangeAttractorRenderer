// GPU-side HCA attractor (discrete map, 2-D coupled quadratic maps).
//
// Params in p[]:
//   p[0].x = a,  p[0].y = c
//
// Equations (xa = previous x):
//   x' = 1 - a*xa^2 + c*(y^2 - xa^2)
//   y' = 1 - a*y^2 + c*(xa^2 - y^2)
...
fn main(@builtin(global_invocation_id) gid: vec3<u32>) {
    ...
    for (var i = 0u; i < params.steps; i++) {
        ...
        if abs(x) > 1e6 || abs(y) > 1e6 {
            ...
            y = 0.5 + (recovery_hash(traj ^ 0x9E3779B9u) - 0.5) * 0.2;
            for (var w = 0u; w < 200u; w++) {
                let wxa = x;
                x = 1.0 - a*wxa*wxa + c*(y*y - wxa*wxa);
                y = 1.0 - a*y*y + c*(wxa*wxa - y*y);
            }
            continue;
        }
