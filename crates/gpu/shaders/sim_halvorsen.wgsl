// GPU-side Halvorsen attractor simulation.
//
// Params in p[]:
//   p[0].x = a, p[0].y = dt
//
...
fn main(@builtin(global_invocation_id) gid: vec3<u32>) {
    ...
    for (var i = 0u; i < params.steps; i++) {
        let vx = -a*x - 4.0*y - 4.0*z - y*y;
        let vy = -a*y - 4.0*z - 4.0*x - z*z;
        let vz = -a*z - 4.0*x - 4.0*y - x*x;
        ...
        if abs(x) > 1e6 || abs(y) > 1e6 || abs(z) > 1e6 {
            ...
            for (var w = 0u; w < 500u; w++) {
                ...
                x = wx + dt * (-a*wx - 4.0*wy - 4.0*wz - wy*wy);
                y = wy + dt * (-a*wy - 4.0*wz - 4.0*wx - wz*wz);
                z = wz + dt * (-a*wz - 4.0*wx - 4.0*wy - wx*wx);
            }
            continue;
        }
