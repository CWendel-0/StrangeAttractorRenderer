// GPU-side Bouali attractor simulation.
//
// Params in p[]:
//   p[0].x = a, p[0].y = s, p[0].z = dt
//
// Equations:
//   x' = x + dt*( x*(4-y) + a*z )
//   y' = y + dt*( -y*(1-x²) )
    ...
    for (var i = 0u; i < params.steps; i++) {
        ...
        if abs(x) > 1e6 || abs(y) > 1e6 || abs(z) > 1e6 {
            ...
            for (var w = 0u; w < 500u; w++) {
                ...
                x = wx + dt * (wx*(4.0-wy) + a*wz);
                y = wy + dt * (-wy*(1.0-wx*wx));
                z = wz + dt * (-wx*(1.5-s*wz) - 0.05*wz);
            }
            continue;
        }
