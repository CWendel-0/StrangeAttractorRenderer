// GPU-side Nishikawa-Kaneko map (discrete 2-D map: logistic map coupled to a circle map).
//
// Params in p[]:
...
fn main(@builtin(global_invocation_id) gid: vec3<u32>) {
    ...
    for (var i = 0u; i < params.steps; i++) {
        let nx = a * x * (1.0 - x) + b * sin(2.0 * PI * y);
        var ny = (y + c) % 1.0;
        if ny < 0.0 { ny = ny + 1.0; }
        ...
        if abs(x) > 1e6 {
            ...
            y = recovery_hash(traj ^ 0x9E3779B9u);
            for (var w = 0u; w < 200u; w++) {
                let wx = x; let wy = y;
                x = a * wx * (1.0 - wx) + b * sin(2.0 * PI * wy);
                var wny = (wy + c) % 1.0;
                if wny < 0.0 { wny = wny + 1.0; }
                y = wny;
            }
            continue;
        }
