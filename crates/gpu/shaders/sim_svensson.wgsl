// GPU-side Svensson attractor (discrete map, 2-D).
//
// Params in p[]:
...
fn map_step(x: f32, y: f32, a: f32, b: f32, c: f32, d: f32) -> vec2<f32> {
    return vec2<f32>(d * sin(a * x) - sin(b * y), c * cos(a * x) + cos(b * y));
}
...
fn main(@builtin(global_invocation_id) gid: vec3<u32>) {
    ...
    for (var i = 0u; i < params.steps; i++) {
        ...
        if abs(x) > 1e6 || abs(y) > 1e6 {
            ...
            y = recovery_hash(traj ^ 0x9E3779B9u) * 0.1;
            for (var w = 0u; w < 200u; w++) {
                let n2 = map_step(x, y, a, b, c, d);
                x = n2.x; y = n2.y;
            }
            continue;
        }
