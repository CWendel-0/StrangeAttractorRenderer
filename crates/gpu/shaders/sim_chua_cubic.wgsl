// GPU-side Chua Cubic attractor simulation (continuous ODE, Euler-integrated).
//
// Params in p[]:
//   p[0].x = alpha, p[0].y = beta, p[0].z = c, p[0].w = dt
//
// Equations:
    ...
    for (var i = 0u; i < params.steps; i++) {
        ...
        if abs(x) > 1e6 || abs(y) > 1e6 || abs(z) > 1e6 {
            ...
            for (var w = 0u; w < 500u; w++) {
                ...
                x = wx + dt * (alpha * (wy - wx*wx*wx - c*wx));
                y = wy + dt * (wx - wy + wz);
                z = wz + dt * (-beta * wy);
            }
            continue;
