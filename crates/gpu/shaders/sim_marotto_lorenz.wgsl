// GPU-side Marotto-Lorenz Map (discrete map, 2-D).
//
// Params in p[]:
//   p[0].x = a,  p[0].y = b
//
...
fn map_step(x: f32, y: f32, a: f32, b: f32) -> vec2<f32> {
    let s = a * x + b * y;
    return vec2<f32>((1.0 - s) * s, x);
}
