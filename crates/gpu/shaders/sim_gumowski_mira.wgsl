// GPU-side Gumowski-Mira attractor (discrete map, 2-D).
//
// Params in p[]:
//   p[0].x = a, p[0].y = b
//
// f(t) = b*t + 2*(1-b)*t^2 / (1+t^2)
// x' = y + a*(1 - 0.05*y^2)*y + f(x)
// y' = -x + f(x')
// z' = 0
...
fn gm_f(t: f32, b: f32) -> f32 {
    return b * t + (2.0 * (1.0 - b) * t * t) / (1.0 + t * t);
}
...
fn main(@builtin(global_invocation_id) gid: vec3<u32>) {
    ...
    for (var i = 0u; i < params.steps; i++) {
        let nx = y + a * (1.0 - 0.05 * y * y) * y + gm_f(x, b);
        let ny = -x + gm_f(nx, b);
        ...
        if abs(x) > 1e6 || abs(y) > 1e6 {
            x = (recovery_hash(traj) - 0.5) * 0.2 + 0.1;
            y = (recovery_hash(traj ^ 0x9E3779B9u) - 0.5) * 0.2 + 0.1;
            for (var w = 0u; w < 200u; w++) {
                let wx = x; let wy = y;
                let wnx = wy + a * (1.0 - 0.05 * wy * wy) * wy + gm_f(wx, b);
                x = wnx;
                y = -wx + gm_f(wnx, b);
            }
            continue;
