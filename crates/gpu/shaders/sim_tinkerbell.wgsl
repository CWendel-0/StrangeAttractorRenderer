// GPU-side Tinkerbell Map (discrete map, 2-D).
//
// Params in p[]:
...
fn main(@builtin(global_invocation_id) gid: vec3<u32>) {
    ...
    for (var i = 0u; i < params.steps; i++) {
        ...
        if abs(x) > 1e6 || abs(y) > 1e6 {
            x = (recovery_hash(traj) - 0.5) * 0.2 - 0.72;
            y = (recovery_hash(traj ^ 0x9E3779B9u) - 0.5) * 0.2 - 0.64;
            for (var w = 0u; w < 200u; w++) {
                let wx = x; let wy = y;
                x = wx * wx - wy * wy + a * wx + b * wy;
                y = 2.0 * wx * wy + c * wx + d * wy;
            }
            continue;
        }
