// GPU-side Multi-Chua II attractor simulation.
//
// Params in p[]:
//   p[0].x = alpha, p[0].y = beta, p[0].z = dt
//
// Multi-segment Chua nonlinearity f(x) with 6 fixed breakpoints (constants,
// matching the original COFFEE script which did not expose these as UI
// parameters):
...
const M0: f32 = -0.14285714285714285;
const M1: f32 = 0.2857142857142857;
const M2: f32 = -0.5714285714285714;
const M3: f32 = 0.2857142857142857;
const M4: f32 = -0.5714285714285714;
const M5: f32 = 0.2857142857142857;
const C1: f32 = 1.0;
const C2: f32 = 2.15;
const C3: f32 = 3.6;
const C4: f32 = 8.2;
...
fn f_nonlinear(x: f32) -> f32 {
    var sum = 0.0;
    sum += (M0 - M1) * (abs(x + C1) - abs(x - C1));
    sum += (M1 - M2) * (abs(x + C2) - abs(x - C2));
    sum += (M2 - M3) * (abs(x + C3) - abs(x - C3));
    sum += (M3 - M4) * (abs(x + C4) - abs(x - C4));
    sum += (M4 - M5) * (abs(x + C5) - abs(x - C5));
    return M5 * x + 0.5 * sum;
}
...
fn main(@builtin(global_invocation_id) gid: vec3<u32>) {
    ...
    let alpha = params.p[0].x;
    let beta  = params.p[0].y;
    let dt    = params.p[0].z;

    for (var i = 0u; i < params.steps; i++) {
        let vx = alpha * (y - f_nonlinear(x));
        let vy = x - y + z;
        let vz = -beta * y;
        ...
        if abs(x) > 1e6 || abs(y) > 1e6 || abs(z) > 1e6 {
            ...
            y = -0.2 + (recovery_hash(traj ^ 0x9E3779B9u) - 0.5) * 0.2;
            z = 0.3 + (recovery_hash(traj ^ 0x85EBCA6Bu) - 0.5) * 0.2;
            for (var w = 0u; w < 500u; w++) {
                let wx = x; let wy = y; let wz = z;
                x = wx + dt * (alpha * (wy - f_nonlinear(wx)));
                y = wy + dt * (wx - wy + wz);
                z = wz + dt * (-beta * wy);
            }
            continue;
