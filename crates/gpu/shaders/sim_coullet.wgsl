// GPU-side Coullet attractor simulation.
//
// Params in p[]:
...
fn main(@builtin(global_invocation_id) gid: vec3<u32>) {
    ...
    for (var i = 0u; i < params.steps; i++) {
        ...
        if abs(x) > 1e6 || abs(y) > 1e6 || abs(z) > 1e6 {
            ...
            y = 0.41 + (recovery_hash(traj ^ 0x9E3779B9u) - 0.5) * 0.2;
            z = 0.31 + (recovery_hash(traj ^ 0x85EBCA6Bu) - 0.5) * 0.2;
            for (var w = 0u; w < 500u; w++) {
                let wx = x; let wy = y; let wz = z;
                x = wx + dt * wy;
                y = wy + dt * wz;
                z = wz + dt * (a*wx + b*wy + c*wz + d*wx*wx*wx);
            }
            continue;
        }
