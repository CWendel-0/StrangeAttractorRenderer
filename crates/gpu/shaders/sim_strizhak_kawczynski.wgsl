// GPU-side Strizhak-Kawczynski attractor simulation.
//
// Params in p[]:
//   p[0] = (a, b, b1, b2), p[1] = (q, r, ax1, ax2), p[2].x = ax3, p[2].y = dt
//
// Equations:
...
fn main(@builtin(global_invocation_id) gid: vec3<u32>) {
    ...
    let q   = params.p[1].x;
    let r   = params.p[1].y;
    let ax1 = params.p[1].z;
    let ax2 = params.p[1].w;
    let ax3 = params.p[2].x;
    let dt  = params.p[2].y;

    for (var i = 0u; i < params.steps; i++) {
        let vx = r*(y - (x-ax1)*(x-ax2)*(x-ax3) - a);
        let vy = b - b1*z - b2*x - y;
        let vz = q*(x - z);
        ...
        if abs(x) > 1e6 || abs(y) > 1e6 || abs(z) > 1e6 {
            ...
            z = (recovery_hash(traj ^ 0x85EBCA6Bu) - 0.5) * 0.2;
            for (var w = 0u; w < 500u; w++) {
                let wx = x; let wy = y; let wz = z;
                x = wx + dt * (r*(wy - (wx-ax1)*(wx-ax2)*(wx-ax3) - a));
                y = wy + dt * (b - b1*wz - b2*wx - wy);
                z = wz + dt * (q*(wx - wz));
            }
            continue;
        }
