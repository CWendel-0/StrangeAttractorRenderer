// GPU-side Sine-Delay attractor (discrete 3rd-order recursion, delay-embedded 2-D).
//
// Params in p[]:
//   p[0].x = a,  p[0].y = b
//
// Equations (state u = x[n-1], v = x[n]):
//   u' = v
//   v' = b*u + a*sin(v)
//   z' = 0
...
fn main(@builtin(global_invocation_id) gid: vec3<u32>) {
    ...
    for (var i = 0u; i < params.steps; i++) {
        ...
        let speed_raw = distance(vec2<f32>(nu, nv), vec2<f32>(u, v));
        let speed_enc = min(u32(log(speed_raw + 1.0) * 32.0), 255u);
        atomicMax(&max_vals[1], speed_enc);

        u = nu; v = nv;

        if abs(u) > 1e6 || abs(v) > 1e6 {
            u = (recovery_hash(traj) - 0.5) * 0.2;
            v = (recovery_hash(traj ^ 0x9E3779B9u) - 0.5) * 0.2;
        ...
        let clip = params.view_proj * vec4<f32>(u, v, z, 1.0);
    ...
    states[traj] = vec4<f32>(u, v, 0.0, bitcast<f32>(rng));
}
