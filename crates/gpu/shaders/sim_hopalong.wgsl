// GPU-side Hopalong attractor (discrete map, 2-D).
//
// Params in p[]:
...
fn main(@builtin(global_invocation_id) gid: vec3<u32>) {
    ...
    for (var i = 0u; i < params.steps; i++) {
        let nx = y - hsign(x) * sqrt(abs(b * x - c));
        let ny = a - x;
        ...
        if abs(x) > 1e6 || abs(y) > 1e6 {
            x = (recovery_hash(traj) - 0.5) * 1.0;
            y = recovery_hash(traj ^ 0x9E3779B9u) * 1.0;
            for (var w = 0u; w < 200u; w++) {
                let wx = x; let wy = y;
                x = wy - hsign(wx) * sqrt(abs(b * wx - c));
                y = a - wx;
            }
            continue;
        }
