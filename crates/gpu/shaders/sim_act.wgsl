// GPU-side ACT attractor simulation.
//
// Params in p[]:
//   p[0].x = a,  p[0].y = b,  p[0].z = d,  p[0].w = m
//   p[1].x = dt
//
// Equations:
//   x' = x + dt*( a*(x-y) )
...
fn main(@builtin(global_invocation_id) gid: vec3<u32>) {
    ...
    for (var i = 0u; i < params.steps; i++) {
        let vx = a * (x - y);
        let vy = -4.0 * a * y + x * z + m * x * x * x;
        let vz = -d * a * z + x * y + b * z * z;
        ...
        if abs(x) > 1e6 || abs(y) > 1e6 || abs(z) > 1e6 {
            ...
            for (var w = 0u; w < 500u; w++) {
                ...
                let dvx = a * (wx - wy);
                let dvy = -4.0 * a * wy + wx * wz + m * wx * wx * wx;
                let dvz = -d * a * wz + wx * wy + b * wz * wz;
                x = wx + dt * dvx;
                y = wy + dt * dvy;
                z = wz + dt * dvz;
            }
