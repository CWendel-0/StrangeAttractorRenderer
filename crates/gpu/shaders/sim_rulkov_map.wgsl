// GPU-side Rulkov Map (discrete map, 2-D).
//
// Params in p[]:
...
fn map_step(x: f32, y: f32, a: f32, b: f32, c: f32) -> vec2<f32> {
    return vec2<f32>(a / (1.0 + x * x) + y, y - b * x - c);
}
...
fn main(@builtin(global_invocation_id) gid: vec3<u32>) {
    ...
    for (var i = 0u; i < params.steps; i++) {
        let n = map_step(x, y, a, b, c);

        let speed_raw = distance(n, vec2<f32>(x, y));
        ...
        if abs(x) > 1e6 || abs(y) > 1e6 {
            x = 1.1 + (recovery_hash(traj) - 0.5) * 0.1;
            y = 1.1 + (recovery_hash(traj ^ 0x9E3779B9u) - 0.5) * 0.1;
            for (var w = 0u; w < 200u; w++) {
                let n2 = map_step(x, y, a, b, c);
                x = n2.x; y = n2.y;
            }
            continue;
        }
    ...
    states[traj] = vec4<f32>(x, y, 0.0, bitcast<f32>(rng));
}
