// GPU-side Popcorn2 attractor (discrete map, 2-D).
//
// Params in p[]:
//   p[0].x = a, p[0].y = b
//
...
fn main(@builtin(global_invocation_id) gid: vec3<u32>) {
    ...
    for (var i = 0u; i < params.steps; i++) {
        let nx = x - a * sin(y + tan(b * y));
        let ny = y - a * sin(nx + tan(b * nx));
        ...
        if abs(x) > 1e6 || abs(y) > 1e6 {
            x = 0.60 + (recovery_hash(traj) - 0.5) * 0.2;
            y = 0.20 + (recovery_hash(traj ^ 0x9E3779B9u) - 0.5) * 0.2;
            for (var w = 0u; w < 200u; w++) {
                let wx = x; let wy = y;
                let wnx = wx - a * sin(wy + tan(b * wy));
                x = wnx;
                y = wy - a * sin(wnx + tan(b * wnx));
            }
            continue;
        }
