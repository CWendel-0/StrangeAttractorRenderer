// GPU-side Aizawa attractor simulation.
//
// Params in p[]:
//   p[0] = (a, b, c, d), p[1].x = e, p[1].y = f, p[1].z = dt
//
// Equations:
//   x' = x + dt*( (z-b)*x - d*y )
//   y' = y + dt*( d*x + (z-b)*y )
...
fn main(@builtin(global_invocation_id) gid: vec3<u32>) {
    ...
    for (var i = 0u; i < params.steps; i++) {
        let vx = (z-b)*x - d*y;
        let vy = d*x + (z-b)*y;
        let vz = c + a*z - (z*z*z)/3.0 - (x*x+y*y)*(1.0+e*z) + f*z*x*x*x;
        ...
        if abs(x) > 1e6 || abs(y) > 1e6 || abs(z) > 1e6 {
            ...
            for (var w = 0u; w < 500u; w++) {
                ...
                x = wx + dt * ((wz-b)*wx - d*wy);
                y = wy + dt * (d*wx + (wz-b)*wy);
                z = wz + dt * (c + a*wz - (wz*wz*wz)/3.0 - (wx*wx+wy*wy)*(1.0+e*wz) + f*wz*wx*wx*wx);
            }
            continue;
        }
