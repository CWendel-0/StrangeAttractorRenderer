// GPU-side Elhadj-Sprott A attractor (discrete map, 2-D).
//
// Params in p[]:
//   p[0].x = a, p[0].y = b
//
// Equations (xa = previous x):
//   x' = 1 - a*y^2 + b*xa
//   y' = xa
...
fn main(@builtin(global_invocation_id) gid: vec3<u32>) {
    ...
    // states[traj].xy holds the emitted point (x, y); .z holds xa (previous x).
    var x = states[traj].x;
    var y = states[traj].y;
    var xa = states[traj].z;
    ...
    for (var i = 0u; i < params.steps; i++) {
        ...
        if abs(x) > 1e6 || abs(y) > 1e6 {
            x = 0.1 + (recovery_hash(traj) - 0.5) * 0.1;
            y = 0.1 + (recovery_hash(traj ^ 0x9E3779B9u) - 0.5) * 0.1;
            xa = x;
            for (var w = 0u; w < 200u; w++) {
                let wy = y;
                let wxa = xa;
                let wnx = 1.0 - a * wy * wy + b * wxa;
                let wny = wxa;
                xa = wnx;
                x = wnx; y = wny;
            }
    ...
    states[traj] = vec4<f32>(x, y, xa, bitcast<f32>(rng));
}
