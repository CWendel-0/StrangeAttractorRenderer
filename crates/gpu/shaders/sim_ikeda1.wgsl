// GPU-side Ikeda 1 attractor simulation (generalized 4-parameter Ikeda map).
//
// Params in p[]:
...
fn main(@builtin(global_invocation_id) gid: vec3<u32>) {
    ...
    for (var i = 0u; i < params.steps; i++) {
        ...
        let nx = a + b * (x * cos(t) - y * sin(t));
        let ny = b * (x * sin(t) + y * cos(t));
        ...
        if abs(x) > 1e6 || abs(y) > 1e6 {
            ...
            for (var w = 0u; w < 200u; w++) {
                let wx = x; let wy = y;
                let wt = c - d / (1.0 + wx * wx + wy * wy);
                x = a + b * (wx * cos(wt) - wy * sin(wt));
                y = b * (wx * sin(wt) + wy * cos(wt));
            }
            continue;
        }
