// GPU-side Heagy-Hammel attractor (discrete map, 2-D).
//
// Params in p[]:
//   p[0].x = a,  p[0].y = s,  p[0].z = b
//
...
fn main(@builtin(global_invocation_id) gid: vec3<u32>) {
    ...
    for (var i = 0u; i < params.steps; i++) {
        let xa = x;
        let nx = a * (1.0 - s * cos(2.0 * PI * y)) * xa * (1.0 - xa);
        let ny = mod1(y + b);
        ...
        if abs(x) > 1e6 || !(y == y) {
            x = recovery_hash(traj);
            y = recovery_hash(traj ^ 0x9E3779B9u);
            for (var w = 0u; w < 200u; w++) {
                let wxa = x;
                x = a * (1.0 - s * cos(2.0 * PI * y)) * wxa * (1.0 - wxa);
                y = mod1(y + b);
            }
            continue;
        }
