// GPU-side Lorenz-84 attractor simulation.
//
// Params in p[]:
//   p[0].x = a, p[0].y = b, p[0].z = f, p[0].w = g, p[1].x = dt
//
// Equations:
//   x' = x + dt*(-a*x - y² - z² + a*f)
...
fn main(@builtin(global_invocation_id) gid: vec3<u32>) {
    ...
    for (var i = 0u; i < params.steps; i++) {
        let vx = -a * x - y * y - z * z + a * f;
        let vy = -y + x * y - b * x * z + g;
        let vz = -z + b * x * y + x * z;
        ...
        if abs(x) > 1e6 || abs(y) > 1e6 || abs(z) > 1e6 {
            ...
            for (var w = 0u; w < 500u; w++) {
                ...
                x = wx + dt * (-a * wx - wy * wy - wz * wz + a * f);
                y = wy + dt * (-wy + wx * wy - b * wx * wz + g);
                z = wz + dt * (-wz + b * wx * wy + wx * wz);
            }
            continue;
        }
