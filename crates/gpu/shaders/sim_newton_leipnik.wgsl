// GPU-side Newton-Leipnik attractor simulation.
//
// Params in p[]:
//   p[0].x = a, p[0].y = b, p[0].z = dt
//
// Equations:
//   x' = x + dt*(-a*x + y + 10*y*z)
...
fn main(@builtin(global_invocation_id) gid: vec3<u32>) {
    ...
    for (var i = 0u; i < params.steps; i++) {
        let vx = -a*x + y + 10.0*y*z;
        let vy = -x - 0.4*y + 5.0*x*z;
        let vz = b*z - 5.0*x*y;
        ...
        if abs(x) > 1e6 || abs(y) > 1e6 || abs(z) > 1e6 {
            x = 0.349 + (recovery_hash(traj) - 0.5) * 0.2;
            y = (recovery_hash(traj ^ 0x9E3779B9u) - 0.5) * 0.2;
            z = -0.160 + (recovery_hash(traj ^ 0x85EBCA6Bu) - 0.5) * 0.2;
            for (var w = 0u; w < 500u; w++) {
                let wx = x; let wy = y; let wz = z;
                x = wx + dt * (-a*wx + wy + 10.0*wy*wz);
                y = wy + dt * (-wx - 0.4*wy + 5.0*wx*wz);
                z = wz + dt * (b*wz - 5.0*wx*wy);
            }
            continue;
        }
