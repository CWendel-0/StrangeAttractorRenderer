// GPU-side Hadley attractor simulation.
//
// Params in p[]:
//   p[0] = (a, b, f, g), p[1].x = dt
//
// Equations:
//   x' = x + dt*( -y² - z² - a*x + a*f )
...
fn main(@builtin(global_invocation_id) gid: vec3<u32>) {
    ...
    for (var i = 0u; i < params.steps; i++) {
        let vx = -y*y - z*z - a*x + a*f;
        let vy = x*y - b*x*z - y + g;
        let vz = b*x*y + x*z - z;
        ...
        if abs(x) > 1e6 || abs(y) > 1e6 || abs(z) > 1e6 {
            ...
            for (var w = 0u; w < 500u; w++) {
                ...
                x = wx + dt * (-wy*wy - wz*wz - a*wx + a*f);
                y = wy + dt * (wx*wy - b*wx*wz - wy + g);
                z = wz + dt * (b*wx*wy + wx*wz - wz);
            }
            continue;
        }
