// GPU-side Cathala attractor (discrete map, 2-D).
//
// Params in p[]:
//   p[0].x = a,  p[0].y = b
//
// Equations:
//   x' = a*x + y
//   y' = b + x²   (uses x value BEFORE this iteration's update)
...
fn main(@builtin(global_invocation_id) gid: vec3<u32>) {
    ...
    for (var i = 0u; i < params.steps; i++) {
        ...
        if abs(x) > 1e6 || abs(y) > 1e6 {
            x = 0.50 + (recovery_hash(traj) - 0.5) * 0.2;
            y = 0.50 + (recovery_hash(traj ^ 0x9E3779B9u) - 0.5) * 0.2;
