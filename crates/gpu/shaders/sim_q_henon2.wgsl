// GPU-side Quasiperiodic Henon map, variant 2 (3-variable skew-product map).
//
// Params in p[]:
...
fn main(@builtin(global_invocation_id) gid: vec3<u32>) {
    ...
    for (var i = 0u; i < params.steps; i++) {
        let nx = a - x * x + b * y + c * cos(2.0 * PI * w);
        let ny = x;
        var nw = (w + d) % 1.0;
        if nw < 0.0 { nw = nw + 1.0; }

        let speed_raw = distance(vec3<f32>(nx, ny, nw), vec3<f32>(x, y, w));
        ...
        if abs(x) > 1e6 || abs(y) > 1e6 {
            x = (recovery_hash(traj) - 0.5) * 0.2;
            y = (recovery_hash(traj ^ 0x9E3779B9u) - 0.5) * 0.2;
            w = recovery_hash(traj ^ 0x85EBCA6Bu);
            for (var k = 0u; k < 200u; k++) {
                let wx = x; let wy = y; let ww = w;
                x = a - wx * wx + b * wy + c * cos(2.0 * PI * ww);
                y = wx;
                var nww = (ww + d) % 1.0;
                if nww < 0.0 { nww = nww + 1.0; }
                w = nww;
            }
            continue;
        }

        let clip = params.view_proj * vec4<f32>(x, y, 0.0, 1.0);
    ...
    states[traj] = vec4<f32>(x, y, w, bitcast<f32>(rng));
}
