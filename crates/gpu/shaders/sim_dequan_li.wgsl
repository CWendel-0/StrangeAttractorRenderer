// GPU-side Dequan Li attractor simulation.
//
// Params in p[]:
//   p[0] = (a, c, d, e), p[1].x = k, p[1].y = f, p[1].z = dt
//
// Equations:
//   x' = x + dt*( a*(y-x) + d*x*z )
//   y' = y + dt*( k*x + f*y - x*z )
    ...
    for (var i = 0u; i < params.steps; i++) {
        ...
        if abs(x) > 1e6 || abs(y) > 1e6 || abs(z) > 1e6 {
            ...
            for (var w = 0u; w < 500u; w++) {
                ...
                x = wx + dt * (a*(wy-wx) + d*wx*wz);
                y = wy + dt * (k*wx + f*wy - wx*wz);
                z = wz + dt * (c*wz + wx*wy - e*wx*wx);
            }
            continue;
        }
