// GPU-side Anishchenko-Astakhov attractor simulation.
//
// Params in p[]:
//   p[0].x = m,  p[0].y = g,  p[0].z = dt
//
// Equations, with step function f(x) = 1 if x>0 else 0:
//   x' = x + dt*( m*x + y - x*z )
//   y' = y + dt*( -x )
    ...
    for (var i = 0u; i < params.steps; i++) {
        ...
        if abs(x) > 1e6 || abs(y) > 1e6 || abs(z) > 1e6 {
            ...
            for (var w = 0u; w < 500u; w++) {
                ...
                let wf = select(0.0, 1.0, wx > 0.0);
                let dvx = m * wx + wy - wx * wz;
                let dvy = -wx;
                let dvz = -g * wz + g * wf * wx * wx;
                x = wx + dt * dvx;
                y = wy + dt * dvy;
                z = wz + dt * dvz;
            }
