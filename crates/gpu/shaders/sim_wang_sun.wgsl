// GPU-side Wang-Sun attractor simulation.
//
// Params in p[]:
//   p[0].x = a, p[0].y = b, p[0].z = c, p[0].w = d
//   p[1].x = e, p[1].y = f, p[1].z = dt
//
// Equations:
//   x' = x + dt*(a*x + c*y*z)
//   y' = y + dt*(b*x + d*y - x*z)
...
fn main(@builtin(global_invocation_id) gid: vec3<u32>) {
    ...
    for (var i = 0u; i < params.steps; i++) {
        ...
        if abs(x) > 1e6 || abs(y) > 1e6 || abs(z) > 1e6 {
            x = 0.30 + (recovery_hash(traj) - 0.5) * 0.2;
            y = 0.10 + (recovery_hash(traj ^ 0x9E3779B9u) - 0.5) * 0.2;
            z = 1.00 + (recovery_hash(traj ^ 0x85EBCA6Bu) - 0.5) * 0.2;
            for (var w = 0u; w < 500u; w++) {
                let wx = x; let wy = y; let wz = z;
                x = wx + dt * (a*wx + c*wy*wz);
                y = wy + dt * (b*wx + d*wy - wx*wz);
                z = wz + dt * (e*wz + f*wx*wy);
            }
            continue;
        }
