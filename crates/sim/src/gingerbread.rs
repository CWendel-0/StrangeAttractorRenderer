...
//   z' = 0
//
// Source: Gingerbread.cof — a=-1.0, b=-1.0, start (4.0, -2.10).
const DESCS: &[ParamDesc] = &[
    ParamDesc { name: "a", kind: ParamKind::Continuous, min: -3.0, max: 3.0, default: -1.0 },
    ParamDesc { name: "b", kind: ParamKind::Continuous, min: -3.0, max: 3.0, default: -1.0 },
];
    ...
    fn warm_up(&mut self) {
        for _ in 0..500 {
            ...
            if !self.state.is_finite() {
                self.state = Vec3::new(4.0, -2.10, 0.0);
                break;
            }
        }
    }
...
impl Default for Gingerbread {
    fn default() -> Self { Self::new() }
}

impl Attractor for Gingerbread {
