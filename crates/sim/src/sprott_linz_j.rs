...
const DESCS: &[ParamDesc] = &[
    ParamDesc { name: "a",  kind: ParamKind::Continuous, min: 0.0, max: 6.0,   default: 2.0 },
    ParamDesc { name: "dT", kind: ParamKind::Continuous, min: 0.0, max: 0.03,  default: 0.006 },
];

pub struct SprottLinzJ {
    state:  Vec3,
    params: [f32; 2],
}
...
impl SprottLinzJ {
    ...
    fn euler_step(&mut self) {
        ...
        let y = self.state.y;
        let z = self.state.z;
        self.state.x = x + dt * (a * z);
        self.state.y = y + dt * (-a * y + z);
        self.state.z = z + dt * (-x + y + y * y);
    }
}

impl Default for SprottLinzJ {
    fn default() -> Self { Self::new() }
}

impl Attractor for SprottLinzJ {
