...
const DESCS: &[ParamDesc] = &[
    ParamDesc { name: "a",  kind: ParamKind::Continuous, min: 0.0, max: 9.0,  default: 3.0 },
    ParamDesc { name: "dT", kind: ParamKind::Continuous, min: 0.0, max: 0.05, default: 0.01 },
];

pub struct SprottLinzD {
    state:  Vec3,
    params: [f32; 2],
}
...
impl SprottLinzD {
    ...
    fn euler_step(&mut self) {
        ...
        self.state.z = z + dt * (x * z + a * y * y);
    }
}

impl Default for SprottLinzD {
    fn default() -> Self { Self::new() }
}

impl Attractor for SprottLinzD {
