...
//   x' = x + dt*y
//   y' = y + dt*z
//   z' = z + dt*(-c*x - b*y - a*z + x²)
const DESCS: &[ParamDesc] = &[
    ParamDesc { name: "a",  kind: ParamKind::Continuous, min: 0.0, max: 1.5,  default: 0.44 },
    ParamDesc { name: "b",  kind: ParamKind::Continuous, min: 0.0, max: 3.5,  default: 1.1  },
...
impl GenesioTesi {
    ...
    fn euler_step(&mut self) {
        ...
        self.state.x = x + dt * y;
        self.state.y = y + dt * z;
        self.state.z = z + dt * (-c * x - b * y - a * z + x * x);
    }
}
...
impl Attractor for GenesioTesi {
