...
//   z' = 0
//
// Source: Popcorn.cof — a=0.4, b=3.0, start (0.60, 0.20).
const DESCS: &[ParamDesc] = &[
    ParamDesc { name: "a", kind: ParamKind::Continuous, min: -1.0, max: 1.0, default: 0.4 },
...
impl Popcorn {
    ...
    fn map_step(&mut self) {
        ...
        let nx = a * (y + (b * y).tan()).sin();
        let ny = a * (x + (b * x).tan()).sin();
        self.state.x = nx;
        self.state.y = ny;
...
impl Default for Popcorn {
    fn default() -> Self { Self::new() }
}

impl Attractor for Popcorn {
