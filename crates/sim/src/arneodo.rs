...
//   x' = x + dt*y
//   y' = y + dt*z
//   z' = z + dt*(-a*x - b*y - z + c*x³)
const DESCS: &[ParamDesc] = &[
    ParamDesc { name: "a",  kind: ParamKind::Continuous, min: -14.0, max: 0.0,   default: -5.5 },
    ParamDesc { name: "b",  kind: ParamKind::Continuous, min: 0.0,   max: 10.5,  default: 3.5  },
    ParamDesc { name: "c",  kind: ParamKind::Continuous, min: -2.5,  max: 0.0,   default: -1.0 },
...
impl Arneodo {
    ...
    fn euler_step(&mut self) {
        ...
        self.state.x = x + dt * y;
        self.state.y = y + dt * z;
        self.state.z = z + dt * (-a * x - b * y - z + c * x * x * x);
    }
}

impl Default for Arneodo {
    fn default() -> Self { Self::new() }
}

impl Attractor for Arneodo {
