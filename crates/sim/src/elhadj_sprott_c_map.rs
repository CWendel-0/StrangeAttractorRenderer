...
const DESCS: &[ParamDesc] = &[
    ParamDesc { name: "a", kind: ParamKind::Continuous, min: -5.0, max: 5.0, default: 3.5 },
    ParamDesc { name: "b", kind: ParamKind::Continuous, min: -3.0, max: 3.0, default: 0.30 },
];

pub struct ElhadjSprottCMap {
    state:  Vec3,
    params: [f32; 2],
}

impl ElhadjSprottCMap {
    pub fn new() -> Self {
        let mut s = Self {
    ...
    fn map_step(&mut self) {
        ...
        self.state.x = -a * x * x + y;
        self.state.y = b - x.abs();
        // z stays 0; camera looks down z-axis for 2-D maps
    }
}

impl Default for ElhadjSprottCMap {
    fn default() -> Self { Self::new() }
}

impl Attractor for ElhadjSprottCMap {
